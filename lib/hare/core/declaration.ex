defmodule Hare.Core.Declaration do
  alias Hare.Core.Chan
  alias __MODULE__.{Result, Declarable}
  ...
  defp validate_each([{name, config} | rest]) do
    case Declarable.validate(name, config) do
  ...
  defp perform(chan, {name, config}, %{tags: tags} = result) do
    case Declarable.run(chan, name, config, tags) do
      :ok ->
