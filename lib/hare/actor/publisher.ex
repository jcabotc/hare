defmodule Hare.Actor.Publisher do
  @moduledoc """
  A behaviour module for implementing AMQP publisher processes.

  The `Hare.Actor.Publisher` module provides a way to create processes that hold,
  ...
  An example `Hare.Actor.Publisher` process that only sends every other message:
  ...
  defmodule MyPublisher do
    ...
    def start_link(conn, config, opts \\ []) do
      Hare.Actor.Publisher.start_link(__MODULE__, conn, config, :ok, opts)
    end

    def publish(publisher, payload, routing_key) do
      Hare.Actor.Publisher.publish(publisher, payload, routing_key)
    end
  ...
  When the `Hare.Actor.Publisher` starts with `start_link/5` it runs the `init/1` callback
  ...
  defmacro __using__(_opts \\ []) do
    quote location: :keep do
      @behaviour Hare.Actor.Publisher
  ...
  @doc """
  Starts a `Hare.Actor.Publisher` process linked to the current process.

  This function is used to start a `Hare.Actor.Publisher` process in a supervision
