defmodule Hare.Actor.RPC.Client do
  @moduledoc """
  A behaviour module for implementing AMQP RPC client processes.

  The `Hare.Actor.RPC.Client` module provides a way to create processes that hold,
  ...
  An example `Hare.Actor.RPC.Client` process that performs a RPC request and returns
  ...
  defmodule MyRPCClient do
    ...
    def start_link(conn, config, opts \\ []) do
      Hare.Actor.RPC.Client.start_link(__MODULE__, conn, config, :ok, opts)
    end
  ...
  When the `Hare.Actor.RPC.Client` starts with `start_link/5` it runs the `init/1` callback
  ...
  defmacro __using__(_opts \\ []) do
    quote location: :keep do
      @behaviour Hare.Actor.RPC.Client
  ...
  @doc """
  Starts a `Hare.Actor.RPC.Client` process linked to the current process.

  This function is used to start a `Hare.Actor.RPC.Client` process in a supervision
