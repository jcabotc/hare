defmodule Hare.Actor.Consumer do
  @moduledoc """
  A behaviour module for implementing AMQP consumer processes.

  The `Hare.Actor.Consumer` module provides a way to create processes that hold,
  ...
  An example `Hare.Actor.Consumer` process that ignores and acks messages with the
  ...
  defmodule MyConsumer do
    ...
    def start_link(conn, config, handler) do
      Hare.Actor.Consumer.start_link(__MODULE__, conn, config, handler)
    end
  ...
  When the `Hare.Actor.Consumer` starts with `start_link/5` it runs the `init/1` callback
  ...
  be acknowledged by using `Hare.Actor.Consumer.ack/2`, `Hare.Actor.Consumer.nack/2` or
  `Hare.Actor.Consumer.reject/2`.
  ...
  @doc """
  Starts a `Hare.Actor.Consumer` process linked to the current process.

  This function is used to start a `Hare.Actor.Consumer` process in a supervision
